...
| Notion build adapter | Existing Astro adapter uses `NOTION_TOKEN` and `NOTION_BLOG_POSTS_DB_ID`, then treats Notion `published` as a build-time public state. | Do not reuse this field as automatic-release authorization. The new release manifest must be independent. |
...
4. Add the approved evidence/release fields to the Notion Blog Posts database and migrate at least one reviewed record.
